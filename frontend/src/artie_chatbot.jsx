// frontend/src/artie_chatbot.jsx
import React, { useState, useEffect, useRef } from "react";
import "./index.css"; // Use your global CSS
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      author: "bot",
      text: "What's up! I'm Artie, the digital brain at Artificial Labs. 🤖 Ask me about our workflow, projects, or just say hi!",
      timestamp: new Date(),
    },
  ]);
      ...
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl flex flex-col h-[70vh]">
            ...
            {messages.map((message) => (
              <div
                ...
                className={`flex gap-3 ${
                  message.author === "bot" ? "justify-start" : "justify-end"
                }`}
              >
                {message.author === "bot" && (
                  <div className="w-8 h-8 bg-gray-800 rounded-full flex items-center justify-center text-white text-sm shrink-0">
                    A
                  </div>
                )}
                <div
                ...
                >
                  ...
                  <p
                    className={`text-xs mt-2 opacity-50 ${
                      message.author === "bot" ? "text-right" : "text-left"
                    }`}
                  >
        ...
        <div className="p-4 bg-white border-t border-gray-100">
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => handleSuggestionClick("Tell me about your workflow")}
              className="px-3 py-1 text-sm bg-gray-100 text-indigo-700 rounded-full hover:bg-indigo-100 transition-colors"
            >
              Explain your workflow
            </button>
            <button
              onClick={() => handleSuggestionClick("Show me some projects")}
              className="px-3 py-1 text-sm bg-gray-100 text-indigo-700 rounded-full hover:bg-indigo-100 transition-colors"
            >
